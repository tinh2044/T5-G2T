...
class ImageCLIP(nn.Module):
    ...
    def forward(self, src_input):
        ...
        B, N, C = x.shape
        cls_token = torch.repeat(self.cls_token, '() n d -> b n d', b=B)
...
class Text_Decoder(nn.Module):
    def __init__(self, config):
        ...
        self.text_decoder = T5ForConditionalGeneration.from_pretrained(config['model']['visual_encoder']).get_decoder()
        self.lm_head = T5ForConditionalGeneration.from_pretrained(config['model']['visual_encoder']).get_output_embeddings()
        self.register_buffer("final_logits_bias", torch.zeros((1, T5ForConditionalGeneration.from_pretrained(config['model']['visual_encoder']).model.shared.num_embeddings)))
...
class gloss_free_model(nn.Module):
    def __init__(self, config, args, embed_dim=1024, pretrain=None):
        ...
        self.backbone = FeatureExtracter(frozen=('freeze_backbone', False))
        self.mbart = None
    ...
    def forward(self,src_input, tgt_input ):
        ...
        out = self.mbart(inputs_embeds = inputs_embeds,
                    ...
                    decoder_attention_mask = tgt_input['attention_mask'],
                    return_dict = True,
                    )
        return out['logits']
    ...
    def generate(self,src_input,max_new_tokens,num_beams,decoder_start_token_id ):
        ...
        out = self.mbart.generate(inputs_embeds = inputs_embeds,
                    attention_mask = attention_mask,max_new_tokens=max_new_tokens,num_beams = num_beams,
