...
class T5_Model(nn.Module):
    ...
    def generate(self, gloss_ids, num_beams):
        return self.model_txt.generate(input_ids=gloss_ids, num_beams=num_beams)
...
class GlossTextCLIP(nn.Module):
    def __init__(self, config, embed_dim=1024, task="clip"):
        ...
        self.config = T5Config.from_pretrained(
            config["model"]["transformer"], cache_dir="./"
        )
        if self.task == "clip":
            self.model_gloss = T5_Model(
                self.config, inplanes=embed_dim, planes=embed_dim, lm_head_type="identy"
            )
            self.model_text = TextCLIP(
                self.config, inplanes=embed_dim, planes=embed_dim, head_type="linear"
            )

            self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        else:
            self.model_gloss = T5_Model(
                self.config, inplanes=embed_dim, planes=embed_dim, lm_head_type="linear"
            )
    ...
    def forward_clip(self, src_input):
        ...
        gloss_features, _ = self.model_gloss(
            src_input["gloss_ids"], src_input["attention_mask"]
        )
        text_features, self.encoder_hidden_states = self.model_text(
            src_input["labels"], src_input["labels_attention_mask"]
        )
        ...
        ground_truth = torch.eye(
            logits_per_gloss.shape[0],
            device=logits_per_text.device,
            dtype=logits_per_gloss.dtype,
            requires_grad=False,
        )
    ...
    def forward_g2t(self, src_input):
        ...
        outputs = self.model_gloss(
            input_ids=src_input["gloss_ids"],
    ...
    def generate(self, src_input, num_beams, device=None):
        out = self.model_gloss.generate(
            src_input["gloss_ids"].to(device), num_beams=num_beams
        )
        return out
