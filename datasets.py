...
class G2T_Dataset(Dataset.Dataset):
    ...
    def collate_fn(self, batch):
        ...
        # Tìm độ dài lớn nhất của text trong batch
        text_output = self.tokenizer(text_batch, return_tensors="pt", padding=True, truncation=True)
        max_text_length = text_output['input_ids'].shape[1]  # Lấy chiều dài lớn nhất của text

        # Tokenize gloss với padding theo max_text_length của text
        gloss_output = self.tokenizer(
            gloss_batch, return_tensors="pt", padding="max_length", 
            max_length=max_text_length, truncation=True
        )
